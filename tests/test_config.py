...
def test_valid_config(tmp_path):
    ...
    cfg = RunConfig.create(
        mode="fixed-N", iterations=3, target_repo=str(target), state_dir=str(state)
    )
...
def test_rejects_relative_paths(tmp_path):
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="fixed-N", iterations=1, target_repo="relative/target", state_dir=str(tmp_path)
        )
...
def test_rejects_nonexistent_paths(tmp_path):
    ...
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="fixed-N", iterations=1, target_repo=str(missing), state_dir=str(state)
        )
...
def test_fixed_n_requires_positive_iterations(tmp_path):
    ...
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="fixed-N", iterations=None, target_repo=str(target), state_dir=str(state)
        )
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="fixed-N", iterations=0, target_repo=str(target), state_dir=str(state)
        )
...
def test_until_converged_forbids_iterations(tmp_path):
    ...
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="until-converged", iterations=2, target_repo=str(target), state_dir=str(state)
        )
...
def test_rejects_state_dir_nested_in_target(tmp_path):
    ...
    with pytest.raises(ConfigError):
        RunConfig.create(
            mode="fixed-N", iterations=1, target_repo=str(target), state_dir=str(nested)
        )
...
def test_rejects_unknown_mode(tmp_path):
    ...
    state.mkdir()
    with pytest.raises(ConfigError):
        RunConfig.create(mode="bogus", iterations=1, target_repo=str(target), state_dir=str(state))
...
def test_roundtrip_to_dict(tmp_path):
    ...
    state.mkdir()
    cfg = RunConfig.create(
        mode="until-converged", target_repo=str(target), state_dir=str(state), focus_areas=["tests"]
    )
    d = cfg.to_dict()
