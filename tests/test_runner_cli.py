...
def test_parse_args_until_converged(tmp_path):
    ...
    t.mkdir()
    s.mkdir()
    ns = parse_args(["--mode", "until-converged", "--target-repo", str(t), "--state-dir", str(s)])
    cfg = build_config(ns)
...
def test_build_orchestrator_aborts_on_dirty_target(tmp_path):
    ...
    cfg = RunConfig.create(
        mode="fixed-N", iterations=1, target_repo=str(repo), state_dir=str(state)
    )
...
def test_parse_args_yes_is_alias_for_non_interactive(tmp_path):
    ...
    s.mkdir()
    ns = parse_args(
        ["--mode", "until-converged", "--target-repo", str(t), "--state-dir", str(s), "--yes"]
    )
    assert ns.non_interactive is True
...
def test_parse_args_brief(tmp_path):
    ...
    ns = parse_args(
        [
            ...
            str(s),
            "--brief",
            "speed up realtime_inventory hot path",
        ]
    )
...
def test_load_run_json_round_trips(tmp_path):
    ...
    cfg = RunConfig.create(
        ...
        target_repo=str(t),
        state_dir=str(s),
        focus_areas=["perf", "tests"],
        value_threshold=0.3,
    )
    p = s / "run.json"
    write_state(p, StateHeader("run-1", None, None, None, "scoped", "orchestrator"), cfg.to_dict())
...
def test_load_run_json_accepts_terminal_run_body(tmp_path):
    ...
    t.mkdir()
    s.mkdir()
    cfg = RunConfig.create(mode="fixed-N", iterations=1, target_repo=str(t), state_dir=str(s))
    p = s / "run.json"
    write_state(
        p,
...
def test_main_from_run_json_target_repo_override(tmp_path, monkeypatch):
    ...
    code = runner_mod.main(["--from-run-json", str(s / "run.json"), "--target-repo", str(ws)])
    assert code == 0
