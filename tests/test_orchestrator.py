...
def test_fixed_n_runs_exactly_n_iterations(tmp_path):
    ...
    orch = Orchestrator(cfg, high_planner_fn=high_planner, team_runner_fn=lambda *a, **k: [])
    summary = orch.run()
    assert calls["n"] == 3
...
def test_high_planner_ctx_includes_brief(tmp_path):
    ...
    cfg = _cfg(
        tmp_path, iterations=1, brief="perf: realtime_inventory hot path; behavior-identical only"
    )
    seen = {}
    ...
    Orchestrator(cfg, high_planner_fn=high_planner, team_runner_fn=lambda *a, **k: []).run()
...
def test_ledger_state_survives_resume(tmp_path):
    ...
    orch.run()
    orch2 = Orchestrator(cfg, high_planner_fn=high_planner, team_runner_fn=team_runner)
...
def test_integrate_crash_persists_failed_status(tmp_path):
    ...
    orch = Orchestrator(
        cfg, high_planner_fn=high_planner, team_runner_fn=team_runner, integrate_fn=integrate_fn
    )
    with pytest.raises(RuntimeError):
        orch.run()
    doc = read_state(Path(cfg.state_dir, "run.json"))
...
def test_run_logs_failure_reason_to_progress(tmp_path):
    ...
    with pytest.raises(RuntimeError):
        Orchestrator(cfg, high_planner_fn=boom, team_runner_fn=lambda *a, **k: []).run()
    progress = Path(cfg.state_dir, "progress.md").read_text()
