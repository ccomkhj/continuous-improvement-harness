...
def _cfg(tmp_path):
    ...
    t.mkdir()
    s.mkdir()
    return RunConfig.create(mode="fixed-N", iterations=2, target_repo=str(t), state_dir=str(s))
...
def _write_execution(
    ...
    p = Path(state_dir) / "iterations" / iter_id / "teams" / team_id / "execution.json"
    body = {"commits": commits}
    if branch is not None:
        body["branch"] = branch
    if head_sha is not None:
        body["head_sha"] = head_sha
    write_state(p, StateHeader(run_id, iter_id, team_id, None, status, "team"), body)
...
def test_reconcile_flags_missing_branch(tmp_path):
    ...
    _write_execution(cfg.state_dir, "run-1", "team-99", [{"red_sha": base, "green_sha": base}])
    report = reconcile(cfg, run_id="run-1")
    assert report["resumable"] is False
...
def test_reconcile_flags_missing_commit(tmp_path):
    ...
    _write_execution(
        cfg.state_dir, "run-1", "team-01", [{"red_sha": "0" * 40, "green_sha": base}], head_sha=base
    )
    report = reconcile(cfg, run_id="run-1")
    assert report["resumable"] is False
