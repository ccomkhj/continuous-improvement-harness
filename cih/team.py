...
def run_team(
    ...
    runner,
    verifier: Callable,
    plan_review_retries: int,
    exec_review_retries: int,
    attempt_cap: int,
    base_sha: str = "",
    branch: str = "",
    worktree_path: str = "",
    ...
    for _ in range(plan_review_retries + 1):
        ...
        plan = invoke(runner, contracts["planner"], {"charter": charter, "feedback": feedback})
        review = invoke(runner, contracts["plan-reviewer"], {"charter": charter, "plan": plan})
        if review["approved"]:
            approved = True
            break
    ...
    if not approved:
        return _finish(
            TeamResult(team_id, False, "plan never approved by plan-reviewer", plan=plan)
        )
    ...
    for _ in range(exec_review_retries + 1):
        try:
            ...
            return _finish(TeamResult(team_id, False, "attempt cap exceeded", plan=plan))
        execution = invoke(runner, contracts["executor"], {"charter": charter, "plan": plan})
        ...
        if review["approved"]:
            return _finish(
                TeamResult(
                    team_id, True, "passed", plan=plan, commits=commits, tdd_verdicts=verdicts
                )
            )
