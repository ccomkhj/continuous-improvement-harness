...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--from-run-json", default=None, dest="from_run_json",
...
def _require_paths(ns: argparse.Namespace) -> None:
    ...
    missing = [f for f, v in (("--target-repo", ns.target_repo),
                              ("--state-dir", ns.state_dir)) if v is None]
    if missing:
...
def write_run_json_cmd(argv: list[str]) -> int:
    ...
    write_state(path, StateHeader("run-1", None, None, None, "scoped", "orchestrator"),
                cfg.to_dict())
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ns.from_run_json:
        ...
        overrides = {k: v for k, v in (("target_repo", ns.target_repo),
                                       ("state_dir", ns.state_dir)) if v is not None}
        if overrides:
