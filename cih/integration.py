    ...
    *,
    ...
    def _ensure_int_wt():
        if state["int_wt"] is None:
            ...
            run_git(
                ["worktree", "add", "-b", int_branch, str(int_wt), state["head"]], cwd=repo, log=log
            )
            state["int_wt"] = int_wt
        return state["int_wt"]
    ...
    def _persist(iteration, team_id, result, wt=None):
        ...
        write_state(
            teamdir / "exec_review.json", header, {"passed": result.passed, "reason": result.reason}
        )
    ...
    def team_runner(charters, ctx):
        ...
        for charter in charters:
            ...
            if log:
                log(
                    f"iter {iteration}: team {team_id} started "
            ...
            if team_verifier is None:
                team_verifier = functools.partial(verify_tdd, repo=wt.path, adapter=tdd_adapter)
            try:
                result = run_team(
                    charter=charter,
                    contracts=contracts,
                    ...
                    plan_review_retries=plan_review_retries,
                    exec_review_retries=exec_review_retries,
            ...
            if result.passed:
                pending[team_id] = {"worktree": wt, "charter": charter, "result": result}
                kept.append(wt)
            else:
                mgr.remove(wt)
            results.append(result)
    ...
    def integrate_fn(results, ctx):
        teams = [(tid, pending[tid]["charter"]) for tid in pending if pending[tid]["result"].passed]
        if not teams:
        ...
        def reverify(team_id, current_base):
            ...
            if proc.returncode not in (0, 5):
                return _reject()
            review = invoke(
                runner, contracts["execution-reviewer"], {"team_id": team_id, "merged": True}
            )
        ...
        if outcome.merged:
            ...
            run_git(["update-ref", f"refs/heads/{int_branch}", state["head"]], cwd=repo, log=log)
        return outcome
    ...
    def teardown():
        ...
        if int_wt is not None:
            try:
                run_git(["worktree", "remove", "--force", str(int_wt)], cwd=repo, log=log)
            except GitError:
                pass
        for wt in kept:
            try:
                run_git(["worktree", "remove", "--force", wt.path], cwd=repo, log=log)
            except GitError:
                pass
        try:
